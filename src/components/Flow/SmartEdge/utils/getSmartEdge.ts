...
import {type SVGDrawFunction, svgDrawSmoothLinePath} from './drawSvgPath';
...
export const getSmartEdge = ({
    ...
    try {
        ...
        let {gridRatio = 20, nodePadding = 10} = options;

        gridRatio = toInteger(gridRatio);
        nodePadding = toInteger(nodePadding);

        // We use the node's information to generate bounding boxes for them and the graph
